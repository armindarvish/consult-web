...
#+RESULTS:
: consult-web--multi-update-sync-candidates
...
#+RESULTS:
: consult-web--multi-update-dynamic-candidates
...
#+RESULTS:
: consult-web--multi-update-async-candidates
...
#+RESULTS:
: consult-web--multi-dynamic-collection
...
#+RESULTS:
: consult-web--multi-dynamic-command
            ...
            (dolist (str candidates)
              (when (and (string-match consult--grep-match-regexp str)
                ...
                (let* ((line (propertize (match-string 2 str) 'face 'consult-line-number))
                  ...
                  ;; (when ctx
                  ;;   (add-face-text-property (+ 2 file-len line-len) (length str) 'consult-grep-context 'append str))
                  (push (propertize str :source "Notes" :title query :file file) result)))))
          result))
#+end_src

#+RESULTS:
: consult-web--notes-transform

**** source
#+begin_src emacs-lisp
(consult-web-define-source "Notes"
                           :narrow-char ?n
                           :type 'async
                           :face 'consult-web-engine-source-face
