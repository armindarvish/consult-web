...
"
  (let* ((frame-width-percent (floor (* (frame-width) 0.1)))
         ...
         ;; (domain (and (url-p urlobj) (or (url-domain urlobj)
         ;;                                 (concat (url-host urlobj) ":" (url-port urlobj)))))
         (domain (and (url-p urlobj)  (or (url-domain urlobj) (url-host urlobj))))
         (port (and (url-p urlobj) (url-portspec urlobj)))
         (domain (if port (format "%s:%s" domain port)))
